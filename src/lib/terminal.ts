...
const routes: Record<string, string> = {
  field: '/field/', systems: '/systems/', notes: '/notes/', photos: '/photography/',
...
export function parseCommand(input: string, projects: TerminalProject[]): TerminalAction {
  ...
  if (!cmd || cmd === 'help') return { kind: 'output', lines: [
    'work        project registry', 'open <id>   open a project', 'field       field records',
  ...
  if (cmd === 'work') return { kind: 'output', lines: projects.map((p) => `${p.id.padEnd(8)} ${p.title.padEnd(24)} ${p.status}`) };
  if (cmd === 'open') {
    ...
    return project
      ...
      : { kind: 'output', lines: [`No project matches: ${args.join(' ') || '(missing id)'}`, 'Try `work` to list projects.'] };
  }
